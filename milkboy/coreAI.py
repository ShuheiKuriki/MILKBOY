...
def choose_cat(word_key, soup=False):
    """
    テーマのカテゴリー選択
    """
    if not soup:
    ...
    for li in li_tags:
        ...
        if "曖昧さ回避" in li_text or li_text in prohibit \
            ...
            or li_text in word_key or li_text in word: continue
        if re.search('.[0-9][0-9]', li_text) is not None: continue
...
def make_replace_words(word_key, texts, true_word):
    ...
    """
    word = re.sub("\(.+?\)", '', word_key) #括弧除去
    word2 = re.sub(" ", '', word) #スペース除去
...
def shape_text(sent, cat, words, subwords, first=False):
    ...
    if len(sent)==0 or sent[-1]=='、': return '', False
    pre = ['これ','それ','この','その','あの','ここ','そこ','こう','そう','以上','上記']
    post = ['以下','下記','下表','※','次の','凡例','参照','別途','記載','述べる','ISBN','出典']
    for word in post:
        if word in sent: return False, False
    tokens = nlp(sent)
    if first and tokens[0].pos_=='ADP': return False, False
    if tokens[-1].pos_=='PUNCT': return False, False
...
def make_feats(cat, word_key, soup, num, true_word=None):
    ...
    for p_tag in texts:
        ...
        doc = text.split('。')
        for sent in doc:
            sent, flag2 = shape_text(sent, cat, replace_words, replace_subwords, feat=='')
    ...
    if first_feat != '': # featXで、featが一つは見つかった場合
        if len(feat_set) >= num-1:
            indices = sorted(sample(range(len(feat_set)), num-1))
            return [first_feat] + list(np.array(feat_set)[indices])
        indices = sorted(sample(range(len(feat_suppli)), min(len(feat_suppli),num-1-len(feat_set))))
        return [first_feat] + feat_set + list(np.array(feat_suppli)[indices])
    if len(feat_set)+len(feat_suppli)==0: return ['']
    if len(feat_set) >= num:
        indices = sorted(sample(range(len(feat_set)), num))
        return list(np.array(feat_set)[indices])
    indices = sorted(sample(range(len(feat_suppli)), min(len(feat_suppli),num-len(feat_set))))
...
def generate_stages(input_theme, theme, anti_themes, cat, seed, stage_max):
    """
    全ステージを生成
    """
    # 特徴文生成
    ...
    for _ in range(stage_num-1):
        stage_dict = {"input_theme": input_theme, "theme": theme, "seed": seed, "category": cat}
...
def make_random(input_theme, seed):
    ...
    oton = soup.find('head').find('title').getText().replace(' - Wikipedia', '')
    return [
        {
        "input_theme": input_theme,
        "theme":"人間の認識能力を超えたもの",
        ...
        "category": "虚無",
        "anti_theme": oton,
        "featX": "オカンが言うには、それは人間の認知能力を超えてるらしいねん",
        "featX_reply": f"人間の認知能力を超えている?!それじゃあ、俺もわからんよ。人間やもの",
        "anti_featX": f"すまん。" ,
        "anti_featX_reply": f"先言えよ！",
        "conjunction": f"申し訳ないなと思って。オトンがいうには、[{oton}] ちゃうかって。",
        "tsukami" : get_tsukami()
        }
    ]

def generate_neta_list(input_theme, seed, stage_max):
    """
    全体処理
    """
    random.seed(seed)
    ...
    if input_theme in ['', 'random']:
        ...
        searched_list = wikipedia.search(input_theme) # 1秒
        # print(searched_list)
        # print("search:",time.time()-t)
        for theme in searched_list:
            try:
                cat, catmems = choose_cat(theme) # 1秒
